...
for target in scraping_targets:
    if 'filename' not in target:
        print(f"Skipping website because 'filename' key is missing in the configuration: {target}")
        continue
    ...
    shade_image_classname=['shade_image_classname']
    ...
    def scrape_data(url):
        ...
        try:
            ...
            for shade_item in shade_items:
                ...
                oprice = "orginal shade price not found"
                if price_parent:
                    try:
...
s3_prefix = 'web_scape_data/' + current_date + '/'
